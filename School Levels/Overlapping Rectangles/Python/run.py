...
class Rectangle:
    ...
    #Each of these methods return True if the position in regard of the input rectangle is met, False if otherwise
    ...
    def isAboveOf(self, otherRectangle):
        return self.bottomRightY > otherRectangle.topLeftY

    def isUnderOf(self, otherRectangle):
        return self.topLeftY < otherRectangle.bottomRightY

    #This method will return 1 or 0 based on the condition for an overlapping rectangle in regards to the input rectangle
    def overlaps(self, r):
        return (1 if not((self.isToRightOf(r)) or (self.isToLeftOf(r)) or (self.isAboveOf(r)) or (self.isUnderOf(r))) else 0)
